...
def main():
    ...
    logging.debug(f"action: config | result: success | logging_level: {logging_level} |"
                  f"n_average_processes: {n_average_processes} ")

    try:
        results_collector = ResultsCollector(n_average_processes)
        results_collector.run()
    except Exception as e:
        logging.error(f'action: run_results_collector | result: fail | error: {str(e)}')
    except:
        logging.error(f'action: run_results_collector | result: fail | error: unknown')
